...
export const fetchPlantPosts = (plantId) => (dispatch) =>
  PostAPIUtil.fetchPlantPosts(plantId).then((posts) =>
    dispatch(receivePosts(posts.data))
  );

export const fetchAllPosts = (keyword) => (dispatch) =>
  PostAPIUtil.fetchAllPosts(keyword).then((posts) =>
    dispatch(receivePosts(posts.data))
  );

export const fetchFollowingPosts = () => (dispatch) =>
  PostAPIUtil.fetchFollowingPosts().then((result) =>
    dispatch(receivePosts(result.data))
  );
