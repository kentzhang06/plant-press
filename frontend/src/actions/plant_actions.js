...
export const fetchPlants = (userId) => (dispatch) =>
  PlantAPIUtil.fetchPlants(userId).then((result) =>
    dispatch(receivedPlants(result.data))
  );

export const fetchPlant = (plantId) => (dispatch) =>
  PlantAPIUtil.fetchPlant(plantId).then((result) =>
    dispatch(receivedPlant(result.data))
  );

export const createPlant = (plantForm) => (dispatch) =>
  PlantAPIUtil.createPlant(plantForm).then(
    (result) => dispatch(receivedPlant(result.data)),
    (err) => dispatch(receivedPlantErrors(err.response.data))
  );

export const updatePlant = (plantForm) => (dispatch) =>
  PlantAPIUtil.updatePlant(plantForm).then(
    (result) => dispatch(receivedPlant(result.data)),
    (err) => dispatch(receivedPlantErrors(err.response.data))
  );

export const deletePlant = (plantId) => (dispatch) =>
  PlantAPIUtil.deletePlant(plantId).then(
